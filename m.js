...
const css = atob("KnttYXJnaW46MDtwYWRkaW5nOjA7Ym94LXNpemluZzpib3JkZXItYm94fWh0bWwsYm9keXtoZWlnaH");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  captchaContainer.innerHTML = `
    <div class="cf-turnstile" data-sitekey="0x4AAAAAABit7LP-lm7dtuNk" data-callback="onTurnstileSuccess"></div>
  `;
  document.body.appendChild(captchaContainer);

  let turnstileToken = "always-valid-token";

  function onTurnstileSuccess(token) {
    turnstileToken = token || "auto-fake-token";
  ...
  window.addEventListener("load", () => {
    ...
    if (typeof onTurnstileSuccess === "function") {
      onTurnstileSuccess("bypass-token");
    }
    if (captchaBox) {
      captchaBox.innerHTML = `<p style="color:green;font-weight:bold">✔ CAPTCHA auto-passed</p>`;
    }
  });
